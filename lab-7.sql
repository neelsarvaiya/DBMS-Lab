...
SELECT * FROM EMPLOYEE WHERE FIRSTNAME LIKE '_[A,E,I,O,U]%';
...
SELECT * FROM EMPLOYEE WHERE CITY LIKE '_[^A,E,I,O,U]%';
...
SELECT * FROM EMPLOYEE WHERE FIRSTNAME LIKE '%[A,E,I,O,U]' AND department LIKE '[A,E,I,O,U]%' AND SALRY between 10000 and 15000;

-- 27. Display employees whose LASTNAME contains ‘A’ at least twice, gender is male, and SALARY is not equalto 14000.
SELECT * FROM EMPLOYEE WHERE LASTNAME LIKE '%A%A%' AND gender = 'MALE' AND SALRY != 14000;

--28. Display employees whose FIRSTNAME second character is vowel and LASTNAME ends with ‘R’ and SALARY less than 12000.
SELECT * FROM EMPLOYEE WHERE FIRSTNAME LIKE '_[A,E,I,O,U]%' AND LASTNAME LIKE '%R' AND SALRY < 12000;

--29. Display employees whose CITY is NOT NULL and FIRSTNAME does not start with vowel and DEPARTMENT not in (‘HR’, ‘IT’).
SELECT * FROM EMPLOYEE WHERE  CITY IS NOT NULL AND FIRSTNAME LIKE '[^A,E,I,O,U]%' AND DEPARTMENT NOT IN (' HR','HR','IT');
